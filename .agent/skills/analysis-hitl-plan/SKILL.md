...
| 区分 | チェック項目 | 判定基準 | NG時の分岐例 |
|------|------------|---------|-------------|
| **必須** | 線形性 `plot(model, 1)` | 残差に系統的パターンなし | 変換 / 非線形項追加 → Gate 2Bへ戻る |
| **必須** | 等分散性 `plot(model, 3)` | 残差の散らばりが一定 | 対数変換 / ロバストSE → Gate 2Bで選択 |
| **必須** | 残差正規性 `plot(model, 2)` | Q-Qプロットが概ね直線 | 大標本なら中心極限定理を根拠に進行可（記載必須） |
| 推奨 | 多重共線性 VIF（`car::vif()` 利用可能なら） | VIF < 5（目安） | Gate 2Cで変数除外感度分析 |

> [!NOTE]
> `car::vif()`, `survminer::ggcoxzph()` 等の追加パッケージは「利用可能なら実施」。
> 利用不可の場合は代替手順（base Rの`plot()`、手動計算など）で対応する。
